...
const { locations, links, coalMarketPrices, ironMarketPrices, incomeTrack, externalPortIds, MARKET_SLOTS, getMarketBuyPrice, getMarketSellPrice } = require('./board-data');
...
function actionBuildLink(state, player, action) {
  ...
  if (state.era === 'canal') {
    ...
    state.log.push({ msg: `${player.username} built canal ${locations[link.from].name} - ${locations[link.to].name}${throughNote}` });
  } else {
    // Rail era
    ...
    if (secondLinkId) {
      ...
      const throughNote = link.through ? ` (via ${link.through})` : '';
      state.log.push({ msg: `${player.username} built rail ${locations[link.from].name} - ${locations[link.to].name}${throughNote}` });
    }
  }
...
function actionSellCotton(state, player, action) {
  ...
  for (const sale of sales) {
    ...
    if (sale.target.type === 'port') {
      ...
      state.log.push({ msg: `${player.username} sold cotton to distant market (demand: ${state.distantMarketDemand}, +${state.distantMarketDemand} income)` });
    }
  }

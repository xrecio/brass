...
function logMsg(state, msg) {
  const era = state.era === 'canal' ? 'C' : 'R';
  return { msg: `[${era}${state.round}] ${msg}`, ts: new Date().toISOString() };
}

function incStr(square) {
  const val = incomeTrack[square];
...
function actionBuildIndustry(state, player, action) {
  const { location, slotIndex, industryType, cardPlayed, wildBuild, secondCard } = action;

  // Validate card(s)
  if (!hasCard(player, cardPlayed)) return { error: 'You don\'t have that card' };
  if (wildBuild) {
    if (!secondCard || !hasCard(player, secondCard)) return { error: 'You need a second card for wild build' };
    if (state.actionsRemaining < 2) return { error: 'Wild build requires 2 actions' };
  }
  ...
  if (slot.owner !== null) {
    // Must build a higher level tile to overbuild
    if (level <= slot.level) {
      return { error: 'Must build a higher level tile to overbuild' };
    }
    if (slot.owner === player.seat) {
      // OK - overbuilding own tile with higher level
    } else if (slot.industryType === 'coalMine' || slot.industryType === 'ironWorks') {
      // Can overbuild opponent coal/iron only if market is empty
  ...
  // Check card validity for this location (skip for wild build — any location allowed)
  if (!wildBuild) {
  ...
  const incomeBefore = player.income;
  player.money -= totalCost;
  player.spentThisRound += totalCost;
  removeCard(player, cardPlayed);
  if (wildBuild && secondCard) {
    removeCard(player, secondCard);
  ...
  const initialCubes = slot.resources;
  let cubesSold = 0;
  let marketEarned = 0;

  // Iron Works: sell to market
  ...
  // Coal Mine: sell to market if connected to port
  ...
  let logParts = [];
  logParts.push(`£${moneyBefore}→spent £${totalCost}→has £${player.money}`);
  if (initialCubes > 0) {
    const cubeType = industryType === 'ironWorks' ? 'iron' : 'coal';
    logParts.push(`${initialCubes} ${cubeType} cubes placed`);
    if (cubesSold > 0) logParts.push(`${cubesSold} sold to market (+£${marketEarned})`);
    if (slot.resources > 0) logParts.push(`${slot.resources} on tile`);
  }
  if (player.income !== incomeBefore) {
    const moved = player.income - incomeBefore;
    logParts.push(`inc +${moved}sq: ${incStr(incomeBefore)}→${incStr(player.income)}/turn`);
  }
  const wildNote = wildBuild ? ' [WILD]' : '';
  state.log.push(logMsg(state, `${player.username} built ${industries[industryType].name} L${level} at ${locations[location].name}${wildNote} (${logParts.join(', ')})`));
  return { success: true };
}
...
function actionBuildLink(state, player, action) {
  ...
  // Must have at least one building on the board to build links
  const hasBuildings = Object.values(state.board.locations).some(loc =>
    loc.slots.some(s => s.owner === player.seat)
  );
  if (!hasBuildings) {
    return { error: 'You need at least one building before building links' };
  }

  // Link must connect to your personal network
  if (!network.has(link.from) && !network.has(link.to)) {
  ...
  if (state.era === 'canal') {
    ...
    state.log.push(logMsg(state, `${player.username} built canal ${locName(link.from)} - ${locName(link.to)}${throughNote} (spent £${canalCost}, has £${player.money})`));
  } else {
    // Rail era
    ...
    if (secondLinkId) {
      ...
      state.log.push(logMsg(state, `${player.username} built 2 rail links (spent £${totalMoney}, has £${player.money})`));
    } else {
      // Building 1 link
      ...
      const throughNote = link.through ? ` (via ${link.through})` : '';
      state.log.push(logMsg(state, `${player.username} built rail ${locName(link.from)} - ${locName(link.to)}${throughNote} (spent £${totalMoney}, has £${player.money})`));
    }
  }
...
function actionSellCotton(state, player, action) {
  ...
  for (const sale of sales) {
    ...
    if (sale.target.type === 'port') {
      ...
      // Flip both — capture income before/after each
      const millOwner = state.players[millSlot.owner];
      const portOwner = state.players[portSlot.owner];
      const millIncBefore = millOwner.income;
      millSlot.flipped = true;
      const millData = industries.cottonMill.levels[millSlot.level];
      adjustIncome(millOwner, millData.incomeGain);

      const portIncBefore = portOwner.income;
      portSlot.flipped = true;
      const portData = industries.port.levels[portSlot.level];
      adjustIncome(portOwner, portData.incomeGain);

      const millInfo = `mill +${millData.incomeGain}sq: ${incStr(millIncBefore)}→${incStr(millOwner.income)}/turn`;
      const portInfo = `port +${portData.incomeGain}sq: ${incStr(portIncBefore)}→${incStr(portOwner.income)}/turn`;
      state.log.push(logMsg(state, `${player.username} sold cotton from ${locations[sale.millLocation].name} via port at ${locations[sale.target.location].name} (${millInfo}, ${portInfo})`));
      ...
      state.log.push(logMsg(state, `${player.username} flipped demand tile: ${tile} (demand now ${state.distantMarketDemand})`));

      if (state.distantMarketDemand <= 0) {
        // Bottomed out - action ends, mill does NOT flip
        state.log.push(logMsg(state, 'Distant market bottomed out! Mill does not flip.'));
        break;
      }

      // Mill flips, gain income bonus equal to remaining demand
      const distMillOwner = state.players[millSlot.owner];
      const distIncBefore = distMillOwner.income;
      millSlot.flipped = true;
      const millData = industries.cottonMill.levels[millSlot.level];
      adjustIncome(distMillOwner, millData.incomeGain);
      const demandBonus = state.distantMarketDemand;
      adjustIncome(player, demandBonus);

      const totalSq = millData.incomeGain + (player.seat === distMillOwner.seat ? demandBonus : 0);
      state.log.push(logMsg(state, `${player.username} sold cotton to distant market (demand: ${state.distantMarketDemand}, mill +${millData.incomeGain}sq +${demandBonus}bonus: ${incStr(distIncBefore)}→${incStr(distMillOwner.income)}/turn)`));
    }
  }
...
function actionTakeLoan(state, player, action) {
  ...
  removeCard(player, cardPlayed);
  const loanDrop = incBefore - player.income;
  state.log.push(logMsg(state, `${player.username} took a loan of £${loanAmount} (has £${player.money}, inc -${loanDrop} sq: ${incStr(incBefore)}/turn→${incStr(player.income)}/turn)`));
  return { success: true };
}
...
function actionDevelop(state, player, action) {
  ...
  for (const industryType of develops) {
    ...
    state.log.push(logMsg(state, `${player.username} developed ${industries[industryType].name} L${removed} (spent £${actualIronResult.money}, has £${player.money})`));
  }
...
function actionPass(state, player, action) {
  ...
  if (!hasCard(player, cardPlayed)) return { error: 'You don\'t have that card' };
  removeCard(player, cardPlayed);
  state.log.push(logMsg(state, `${player.username} passed`));
  return { success: true };
}
...
function endRound(state) {
  state.log.push(logMsg(state, '--- End of Round ' + state.round + ' ---'));
  ...
  for (const player of state.players) {
    ...
    state.log.push(logMsg(state, `Phase 1: ${player.username} collects £${incomeAmount} income (${incStr(player.income)}/turn, £${moneyBefore}→£${player.money})`));
  }
  ...
  // Phase 3: Reorder by spending (ascending - least spent goes first, ties keep order)
  const order = state.players
    .map((p, i) => ({ seat: i, spent: p.spentThisRound, prevIdx: state.turnOrder.indexOf(i) }))
    .sort((a, b) => {
      if (a.spent !== b.spent) return a.spent - b.spent;
      return a.prevIdx - b.prevIdx; // ties keep previous order
    })
    .map(p => p.seat);
  state.turnOrder = order;

  const orderNames = order.map(s => state.players[s].username + '(£' + state.players[s].spentThisRound + ')').join(', ');
  state.log.push(logMsg(state, 'Phase 3: New turn order: ' + orderNames));

  // Phase 4: Empty spent boxes, refill hands
  for (const player of state.players) {
  ...
  for (const seat of state.turnOrder) {
    ...
    cardsDrawn += player.hand.length - before;
  }
  state.log.push(logMsg(state, 'Phase 4: ' + cardsDrawn + ' cards drawn, ' + state.drawPile.length + ' remain in deck. Spent boxes emptied.'));

  state.round++;
  state.currentPlayerIndex = 0;
  // First round of Canal era: 1 action each
  state.actionsRemaining = (state.era === 'canal' && state.round === 1) ? 1 : 2;

  state.log.push(logMsg(state, '=== Round ' + state.round + ' begins ==='));
}
...
function endEra(state) {
  ...
  if (state.era === 'canal') {
    ...
    state.log.push(logMsg(state, '=== Game Over ==='));
    for (const p of state.players) {
      const moneyVP = Math.floor(p.money / 10);
      state.log.push(logMsg(state, `${p.username}: ${p.vp} VP (inc £${p.money} → +${moneyVP} VP)`));
    }

    // Determine winner
    const winner = state.players.reduce((best, p) =>
      p.vp > best.vp ? p : best, state.players[0]);
    state.log.push(logMsg(state, `🏆 ${winner.username} wins with ${winner.vp} VP!`));
  }
}

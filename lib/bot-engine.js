...
function playBotTurn(gameId) {
  ...
  // Add announcement to game log
  state.log.push({ msg: '🤖 ' + description, bot: true });
  ...
  if (result.error) {
    ...
    const passDesc = describeAction(passAction, currentPlayer, state);
    state.log.push({ msg: '🤖 ' + passDesc, bot: true });
  ...
  if (!success) { console.log('[Bot] State save failed (concurrent modification)'); return; }
